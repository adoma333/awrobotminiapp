...
## 10) بوابة الدفع الخاصة AW Pay (بديل NOWPayments)

**الملفات:** `backend/chain_crypto.py` (المفاتيح والتوقيع)، `backend/gw_chains.py` (TRON/BSC/TON)، `backend/gateway.py` (الفواتير والتجميع)، `backend/gateway_keys.py` (أداة السيرفر)، صفحة «بوابة الدفع» في لوحة التحكم.

**التشغيل لأول مرة:**
1. `cd /home/aw/aw-mini-app/backend && .venv/bin/python gateway_keys.py new-master` ← انسخ السطر إلى `.env`، واحفظ نسخة منه خارج السيرفر.
   **لا تغيّره أبدًا بعد أول دفعة**: كل عناوين الإيداع مشتقة منه.
2. (موصى به) مفتاح مجاني من trongrid.io في `.env`: `TRON_API_KEY=...`. واختياري: `BSC_RPC=...`، `TONCENTER_API_KEY=...`.
3. `aw-update` ثم لوحة التحكم ← بوابة الدفع ← «عناوين الاستلام»: اضبط عنوان كل شبكة برمز OTP.
4. `.venv/bin/python gateway_keys.py gas` ← اشحن عنوانَي خزان الغاز (مثلًا 100 TRX و 0.02 BNB).
5. «نظرة عامة» ← «تشغيل البوابة الخاصة». الدفعات القديمة عبر NOWPayments تبقى تُراجَع تلقائيًا حتى تكتمل.

**كيف تعمل:**
- TRON/BSC: عنوان إيداع لكل مستخدم. يُقاس الدفع بفرق الرصيد المؤكَّد، ثم يُجمَّع إلى عنوانك عند بلوغ الحد الأدنى (الغاز من الخزان).
